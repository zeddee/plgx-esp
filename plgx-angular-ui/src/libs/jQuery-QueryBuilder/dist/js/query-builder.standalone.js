...
<div id="{{= it.group_id }}" class="rules-group-container"> \
  <div class="rules-group-header" style="margin-bottom: 28px;"> \
    <div class="btn-group pull-right group-actions"> \
      <button type="button" class="btn btn-sm btn-outline-success" data-add="rule"> \
        <i class="fa fa-plus"></i> {{= it.translate("add_rule") }} \
      </button> \
      {{? it.settings.allow_groups===-1 || it.settings.allow_groups>=it.level }} \
        <button type="button" class="btn btn-sm btn-outline-success" data-add="group"> \
          <i class="fa fa-plus-circle"></i> {{= it.translate("add_group") }} \
        </button> \
      {{?}} \
      {{? it.level>1 }} \
        <button type="button" class="btn btn-sm btn-outline-danger" data-delete="group"> \
          <i class="fa fa-times"></i> {{= it.translate("delete_group") }} \
        </button> \
      {{?}} \
    </div> \
    <div class="btn-group group-conditions"> \
      {{~ it.conditions: condition }} \
        <label class="btn btn-sm btn-outline-info"> \
          <input type="radio" style="border: 0;clip: rect(0 0 0 0);height: 1px;margin: -1px;overflow: hidden;padding: 0;position: absolute;width: 1px;white-space: nowrap;" name="{{= it.group_id }}_cond" value="{{= condition }}"> {{= it.translate("conditions", condition) }} \
        </label> \
      {{~}} \
    </div> \
  ...
  <div class=rules-group-body> \
    <div class="rules-list" style="list-style: none;padding: 0 0 0 15px;margin: 0;"></div> \
  </div> \
</div>';

  QueryBuilder.templates.rule = '\
<div id="{{= it.rule_id }}" class="rule-container" style="position: relative;margin: 4px 0;border-radius: 5px;padding: 5px;border: 1px solid #EEE;background: rgba(255, 255, 255, 0.9);"> \
<span class="first-child" style="top: -33px;height: calc(88% + 14px);position: absolute;left: -10px;width: 10px;border-color: #CCC;border-style: solid;border-width: 0 0 2px 2px;border-radius: 0 0 0 4px;"> </span> \
  <div class="rule-header" style=""> \
    <div class="btn-group pull-right rule-actions"> \
      <button type="button" class="btn btn-sm btn-outline-danger" data-delete="rule"> \
        <i class="fa fa-times"></i> {{= it.translate("delete_rule") }} \
      </button> \
    </div> \
  </div> \
  {{? it.settings.display_errors }} \
    <div class="error-container"><i class="{{= it.icons.error }}"></i></div> \
  {{?}} \
  <div class="rule-filter-container" style="display: inline-block;vertical-align: middle;border: 1px solid #ebedf2;border-radius: 4px;"></div> \
  <div class="rule-operator-container" style="display: inline-block;vertical-align: middle;border: 1px solid #ebedf2;border-radius: 4px;"></div> \
  <div class="rule-value-container" style="margin: inherit;width: 200px;display: block ruby;"></div> \
</div>';
...
<div' + style + ' class="' + filter.input + ' ' + filter.input + '-' + color + '"> \
  ...
  QueryBuilder.define('invert', function(options) {
    ...
    if (!options.disable_template) {
      this.on('getGroupTemplate.filter', function(h) {
        var $h = $(h.value);
        $h.find(Selectors.condition_container).after('<button type="button" class="btn btn-sm btn-default" data-invert="group">' +
      ...
      if (options.display_rules_button && options.invert_rules) {
        this.on('getRuleTemplate.filter', function(h) {
          var $h = $(h.value);
          $h.find(Selectors.rule_actions).prepend('<button type="button" class="btn btn-sm btn-default" data-invert="rule">' +
            '<i class="' + options.icon + '"></i> ' + self.translate('invert') +
            '</button>');
          h.value = $h.prop('outerHTML');
        });
  ...
  QueryBuilder.define('not-group', function(options) {
    ...
    if (!options.disable_template) {
      this.on('getGroupTemplate.filter', function(h) {
        var $h = $(h.value);
        $h.find(QueryBuilder.selectors.condition_container).prepend('<button type="button" class="btn btn-sm btn-default" data-not="group">' +
  ...
  QueryBuilder.extend({
    updateGroupNot: function(group) {
      ...
      group.$el.find('>' + QueryBuilder.selectors.group_not).toggleClass('active', group.not).find('i').attr('class', group.not ? options.icon_checked : options.icon_unchecked);
  ...
  QueryBuilder.define('sortable', function(options) {
    ...
    this.on('afterAddRule afterAddGroup', function(e, node) {
      ...
      if (!node.flags.no_sortable) {
        interact(node.$el[0]).draggable({
          ...
          onstart: function(event) {
            ...
            ghost = src.$el.clone().appendTo(src.$el.parent()).width(src.$el.outerWidth()).addClass('dragging');
            var ph = $('<div class="rule-placeholder">&nbsp;</div>').height(src.$el.outerHeight());
      ...
      if (!node.flags.no_drop) {
        ...
        if (node instanceof Group) {
          interact(node.$el.find(QueryBuilder.selectors.group_header)[0]).dropzone({
            accept: QueryBuilder.selectors.rule_and_group_containers,
            ondragenter: function(event) {
              moveSortableToTarget(placeholder, $(event.target), self);
            },
