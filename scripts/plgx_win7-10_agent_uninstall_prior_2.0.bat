...
REM Use this script to perform uninstall of Polylogyx Agent prior to v2.0 via 
REM Custom Script Execution on server. Update the server IP (line 14) below,
REM agent administrator level username(line 15) and password(line 16) and push
REM script to the agent.
REM
...
SET USERNAME="<PROVIDE_AGENT_USERNAME_HERE>"
SET PASSWORD="<PROVIDE_AGENT_PASSWORD_HERE>"
...
SET ACTION="%OUTPUT% -u s"

schtasks /create /RU %USERNAME% /RP %PASSWORD% /TN %TASK% /TR %ACTION% /RL HIGHEST /SC ONCE /ST %NEWTIME% /F
IF %ERRORLEVEL% NEQ 0 (
