...
class OAuthLogin(object):
    ...
    @property
    def client_id(self):
        return self.config.get('POLYLOGYX_OAUTH_CLIENT_ID')

    @property
    def client_secret(self):
        return self.config.get('POLYLOGYX_OAUTH_CLIENT_SECRET')

    @property
    def base_url(self):
        return self.config.get('POLYLOGYX_OAUTH_GOOGLE_BASE_URL')

    @property
    def redirect_uri(self):
        return url_for(self.config.get('POLYLOGYX_AUTH_REDIRECT_VIEW'),
                       _scheme=self.config.get('PREFERRED_URL_SCHEME'),
                       _external=True)

    @property
    def token_url(self):
        return self.config.get('POLYLOGYX_OAUTH_GOOGLE_TOKEN_URL')

    @property
    def scope(self):
        return self.config.get('POLYLOGYX_OAUTH_GOOGLE_SCOPE')
...
class GoogleOAuthV2Login(OAuthLogin):
    ...
    def init_app(self, app):
        ...
        if not self.config['POLYLOGYX_OAUTH_CLIENT_ID']:
            raise ValueError("POLYLOGYX_OAUTH_CLIENT_ID must be configured")

        if not self.config['POLYLOGYX_OAUTH_CLIENT_SECRET']:
            raise ValueError("POLYLOGYX_OAUTH_CLIENT_SECRET must be configured")

        self.config.setdefault('POLYLOGYX_OAUTH_GOOGLE_BASE_URL',
                               'https://accounts.google.com/o/oauth2/auth')
        self.config.setdefault('POLYLOGYX_OAUTH_GOOGLE_TOKEN_URL',
                               'https://www.googleapis.com/oauth2/v3/token')
        self.config.setdefault('POLYLOGYX_AUTH_REDIRECT_VIEW',
                               'users.oauth2callback')

        self.config.setdefault('POLYLOGYX_OAUTH_GOOGLE_SCOPE', ['email', 'profile'])

        self.config.setdefault('POLYLOGYX_OAUTH_GOOGLE_ALLOWED_DOMAINS', [])
        self.config.setdefault('POLYLOGYX_OAUTH_GOOGLE_ALLOWED_USERS', [])

        app.oauth_provider = self

    @property
    def allowed_users(self):
        return self.config.get('POLYLOGYX_OAUTH_GOOGLE_ALLOWED_USERS', [])

    @property
    def allowed_domains(self):
        return self.config.get('POLYLOGYX_OAUTH_GOOGLE_ALLOWED_DOMAINS', [])
